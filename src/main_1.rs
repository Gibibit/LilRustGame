...
impl Pseudocube {
	...
	pub fn update_eye_position(&mut self, mouse_x: f32, mouse_y: f32) {
		...
		let pos = (
			...
			fclamp(mouse_y, cpos.1, cpos.1 + csize.1 - self.eye.size.1),
		);
...
pub fn main() {
    ...
    let pso = factory.create_pipeline_simple(
	    include_bytes!(concat!(env!("CARGO_MANIFEST_DIR"), "/shaders/rect_150_1.glslv")),
	    include_bytes!(concat!(env!("CARGO_MANIFEST_DIR"), "/shaders/rect_150_1.glslf")),
	    pipe::new()
	).unwrap();
